...
- PDF generation: `generar_pdf(datos, None)` in `documento_service.py` - reads from `datos` dict, NOT from IA-generated text; writes to `{cedula}.pdf` (`path_tutela_pdf_nombre`, sin colisiones) — ese archivo es el que se sube como DEMANDA/PRUEBA al portal. El envío por WhatsApp y la descarga del admin usan `{cedula}.pdf` (no `tutela_{id}.pdf`).
